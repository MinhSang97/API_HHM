...
func (s *studentRepository) CheckLogin(ctx context.Context, user, password string) (model.Login, error) {
	var Tokens model.Login
	if err := s.db.Where("user = ? AND password = ?", user, password).First(&Tokens).Error; err != nil {
		return Tokens, fmt.Errorf("check login error: %w", err)
	}
	return Tokens, nil
}
